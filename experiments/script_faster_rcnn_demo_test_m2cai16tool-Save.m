...
frames = dir(strcat(pwd, '/test_input_aj/m2cai16-tool/tool_video_11/*.jpg')); % ADDED, frames is structure array with fields name, date, bytes
...
%    im = imread(fullfile(strcat(pwd, '/test_input_aj/HighlyRatedTrimResize'), im_names{j})); % CHANGED pwd part
    im = imread(fullfile(strcat(pwd, '/test_input_aj/m2cai16-tool/tool_video_11'), im_names{j})); % CHANGED pwd part
    ...
    %thres = 0.6;
    thres = 0.0; % CHANGED for m2cai16-tool test
...
%    fig=openfig(strcat('/home/ajin/faster_rcnn/test_output_aj/HighlyRatedTrimResize/', im_names{j}(1:end-4), '_result.fig'),'new','invisible');
 %   saveas(fig,strcat('/home/ajin/faster_rcnn/test_output_aj/HighlyRatedTrimResize/', im_names{j}(1:end-4), '_result.jpg'),'jpg')
    ...
    savefig(strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11/', im_names{j}(1:end-4), '_result.fig')) % ADDED
    fig=openfig(strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11/', im_names{j}(1:end-4), '_result.fig'),'new','invisible');
    saveas(fig,strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11/', im_names{j}(1:end-4), '_result.jpg'),'jpg')

    close(fig);
    pause(0.1);
end
    annot_filename = strcat('image', '_annot.csv'); %added
    writetable(struct2table(annot3), strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11/', annot_filename)); %added
