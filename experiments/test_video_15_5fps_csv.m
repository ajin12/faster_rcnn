...
%frames = dir(strcat(pwd, '/test_input_aj/m2cai16-tool/tool_video_11_part1/*.jpg')); % ADDED, frames is structure array with fields name, date, bytes
frames = dir(strcat('/vision/u/ajin/m2cai16-tool/VideoFrames-5fps-subfolders', '/tool_video_15_05/*.jpg'));
...
%    im = imread(fullfile(strcat(pwd, '/test_input_aj/HighlyRatedTrimResize'), im_names{j})); % CHANGED pwd part
   % im = imread(fullfile(strcat(pwd, '/test_input_aj/m2cai16-tool/tool_video_11_part1'), im_names{j})); % CHANGED pwd part
   im = imread(fullfile(strcat('/vision/u/ajin/m2cai16-tool/VideoFrames-5fps-subfolders', '/tool_video_15_05'), im_names{j})); % CHANGED pwd part
    ...
    %thres = 0.6; % 8/16/2017 use threshold 0.6 to test MoreAnnotations1-7-26-2017 model
    %thres = 0.5; % 1/25/2017 Use threshold 0.5 for testing
    %thres = 0.00001; % CHANGED for m2cai16-tool test (was 0.0 for 8Classes test)
    thres = 0.8; % 2/8/2017  Use threshold 0.8 for testing. 8/15/2017 Use 0.8 threshold to test MoreAnnotations1-7-26-2017 model
    %thres = 0.9; 
...
%    fig=openfig(strcat('/home/ajin/faster_rcnn/test_output_aj/HighlyRatedTrimResize/', im_names{j}(1:end-4), '_result.fig'),'new','invisible');
 %   saveas(fig,strcat('/home/ajin/faster_rcnn/test_output_aj/HighlyRatedTrimResize/', im_names{j}(1:end-4), '_result.jpg'),'jpg')
    ...
    %savefig(strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11_part1/', im_names{j}(1:end-4), '_result.fig')) % ADDED
    %fig=openfig(strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11_part1/', im_names{j}(1:end-4), '_result.fig'),'new','invisible');
    %saveas(fig,strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11_part1/', im_names{j}(1:end-4), '_result.jpg'),'jpg')

    %close(fig);
    pause(0.1);
end
    %annot_filename = strcat('tool_video_11_part1_image', '_test.csv'); %added
    %writetable(struct2table(annot3), strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_11_part1/', annot_filename)); %added

    annot_filename = strcat('tool_video_15_05', '_5fps_test.csv'); %added
    writetable(struct2table(annot3), strcat('/vision/u/ajin/m2cai16-tool/test_output_aj/csv-5fps-threshold-0.8-tested-against-MoreAnnotations1-7-26-2017-model/', annot_filename)); 
