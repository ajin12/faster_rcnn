...
%frames = dir(strcat(pwd, '/test_input_aj/m2cai16-tool/tool_video_12/*.jpg')); % ADDED, frames is structure array with fields name, date, bytes

frames = dir(strcat(pwd, '/test_input_aj/cholec80/video04/*.jpg'));
...
%    im = imread(fullfile(strcat(pwd, '/test_input_aj/HighlyRatedTrimResize'), im_names{j})); % CHANGED pwd part
    im = imread(fullfile(strcat(pwd, '/test_input_aj/cholec80/video04'), im_names{j})); % CHANGED pwd part
    ...
    thres = 0.8; % 2/8/2017  Use threshold 0.8 for testing. 11/28/2017 use 0.8 for wacv test
    %thres = 0.9; 
...
%    fig=openfig(strcat('/home/ajin/faster_rcnn/test_output_aj/HighlyRatedTrimResize/', im_names{j}(1:end-4), '_result.fig'),'new','invisible');
 %   saveas(fig,strcat('/home/ajin/faster_rcnn/test_output_aj/HighlyRatedTrimResize/', im_names{j}(1:end-4), '_result.jpg'),'jpg')
    ...
    %savefig(strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_12/', im_names{j}(1:end-4), '_result.fig')) % ADDED
    %fig=openfig(strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_12/', im_names{j}(1:end-4), '_result.fig'),'new','invisible');
    %saveas(fig,strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_12/', im_names{j}(1:end-4), '_result.jpg'),'jpg')

    %close(fig);
    pause(0.1);
end
   % annot_filename = strcat('tool_video_12_image', '_test.csv'); %added
   % writetable(struct2table(annot3), strcat('/home/ajin/faster_rcnn/test_output_aj/m2cai16-tool/tool_video_12/', annot_filename)); %added

    annot_filename = strcat('cholec80_video04', '_test.csv'); %added
    writetable(struct2table(annot3), strcat('/home/ajin/faster_rcnn/test_output_aj/cholec80/', annot_filename)); %added
